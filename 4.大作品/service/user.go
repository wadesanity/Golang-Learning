...
func (*userService) Register(ctx context.Context, req *types.UserRegisterReq) (*model.User, error) {
	...
	if err != nil {
		util2.Logger.Errorf("用户注册方法->用户存在方法->error:%v,请求形参:%v", err, req.Name)
		return nil, e.NewApiError(http.StatusInternalServerError, e.DbQueryError.Error())
	}
	if b > 0 {
		util2.Logger.Errorf("用户注册方法->用户存在方法->already found,请求参数:%v", req.Name)
	...
	if err != nil {
		util2.Logger.Errorf("用户注册方法->用户添加方法错误:%v,请求形参:%#v,", err, user)
...
func (*userService) Login(ctx context.Context, req *types.UserLoginReq) (string, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			util2.Logger.Errorf("用户登录方法->用户查询方法->not found,请求形参:%v", req.Name)
			return "", e.NewApiError(http.StatusNotFound, e.DbQueryNotFound.Error())
		}
		util2.Logger.Errorf("用户登录方法->用户查询方法->error:%v,请求形参:%v", err, req.Name)
	...
	return util2.NewJwt(user.ID, isAuditor), nil
...
func (*userService) ChangePwd(ctx context.Context, req *types.UserChangePwdReq) (*time.Time, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			util2.Logger.Errorf("用户修改密码方法->用户查询方法->not found,用户id:%v", req.ID)
			return nil, e.NewApiError(http.StatusNotFound, e.DbQueryNotFound.Error())
		}
		util2.Logger.Errorf("用户修改密码方法->用户查询方法->error:%v,用户id:%v", err, req.ID)
		return nil, e.NewApiError(http.StatusInternalServerError, e.DbQueryError.Error())
	}

	if !user.CheckPwd(req.PwdOld) {
		util2.Logger.Errorf("用户修改密码方法->密码校验->false,用户名:%v", req.ID)
	...
	if err != nil {
		util2.Logger.Errorf("用户修改密码方法->用户修改方法->error:%v,用户model:%#v", err, userNew)
...
func (*userService) ShowInfo(ctx context.Context, userID uint) (*model.User, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			util2.Logger.Errorf("用户信息展示方法->not found,用户id:%v", userID)
			return nil, e.NewApiError(http.StatusNotFound, e.DbQueryNotFound.Error())
		}
		util2.Logger.Errorf("用户信息展示方法->error:%v,用户id:%v", err, userID)
		return nil, e.NewApiError(http.StatusInternalServerError, e.DbQueryError.Error())
	}
	return user, nil
}
...
func (*userService) ChangeAvatar(ctx context.Context, req *types.UserChangeAvatarReq) (*model.User, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			util2.Logger.Errorf("用户修改头像方法->查询->not found,用户id:%v", req.ID)
			return nil, e.NewApiError(http.StatusNotFound, e.DbQueryNotFound.Error())
		}
		util2.Logger.Errorf("用户修改头像方法->查询->error:%v,用户id:%v", err, req.ID)
		return nil, e.NewApiError(http.StatusInternalServerError, e.DbQueryError.Error())
	}
	...
	userNew, err = userDao.ChangeByModel(userNew)
	if err != nil {
		util2.Logger.Errorf("用户修改头像方法->修改->error:%v", err)
		return nil, e.NewApiError(http.StatusInternalServerError, e.DbUpdateError.Error())
	}
	return userNew, nil
}
...
func (*userService) List(ctx context.Context, req *types.UserListReq) (any, int64, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			util2.Logger.Debugf("user list total req:%#v, not found", req)
			return nil, total, nil
		}
		util2.Logger.Errorf("user list total err:%v", err)
		return nil, total, e.NewApiError(http.StatusInternalServerError, e.DbQueryError.Error())
	}
	util2.Logger.Debugf("res:%#v, %v", res, res == nil)
	if total == 0 {
		return nil, total, nil
	}
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			util2.Logger.Debugf("user list req:%#v, not found", req)
			return nil, total, nil
		}
		util2.Logger.Errorf("user list err:%v", err)
		return nil, total, e.NewApiError(http.StatusInternalServerError, e.DbQueryError.Error())
	}
